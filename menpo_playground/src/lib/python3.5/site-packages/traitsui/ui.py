...
class UI ( HasPrivateTraits ):
    ...
    def finish ( self ):
        ...
        for object in list(self.context.values()):
    ...
    def prepare_ui ( self ):
        ...
        if (len( self._visible ) +
            len( self._enabled ) +
            len( self._checked )) > 0:
            for object in list(context.values()):
    ...
    def sync_view ( self ):
        """ Synchronize context object traits with view editor traits.
        """
        for name, object in list(self.context.items()):
    ...
    def _sync_view ( self, name, object, metadata, direction ):
        info = self.info
        for trait_name, trait in list(object.traits( **{metadata: is_str} ).items()):
